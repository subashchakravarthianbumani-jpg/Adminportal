...
const ApprovedTable = ({ updatedValuesPage }) => {
  ...
  const fetchRegistrations = async (pageIndex = 0, pageSize = 10) => {
    ...
    try {
      ...
      const sortingPayload = sortingState.id
        ...
        const requestData = {
          skip,
            ...
            columnSearch: [],
            where: {"RegistrationType": ["EXHIBITORS"],"IsStallApproved": [1]}
      ...
      console.log("--------register-----/n",registrationData.data);
  const registrationTable = registrationData.data
...
const formatDateTime = (value) => {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <>
            <Button variant='contained' color='primary' size='small' onClick={() => handleBookStall(row.original)}>
              Book Stall
            </Button>
          </>
        )
  ...
  const exportToPdf = () => {
    ...
    doc.text('Exhibitors Approved', 10, 10)
    ...
    doc.save('Exhibitors Approved.pdf')
  }
  ...
  const exportToExcel = () => {
    ...
    // Trigger download
    XLSX.writeFile(workbook, 'Exhibitors Approved.xlsx')
  }
  ...
  // Function to handle stall booking

  const handleBookStall = async rowData => {
    // alert(rowIds.id)

    const confirmApprove = await Swal.fire({
      title: 'Do you want to book stall this registration?',
      text: '',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, Book Stall',
        ...
        return (
          <>
            <Card>
              ...
              <Button
                onClick={() => {
                  // setSectionHandler('table'); // Ensure state is updated
                  navigate('dashboards/stallallocation') // Navigate to the desired path
                }}
              >
                Back
              </Button>
