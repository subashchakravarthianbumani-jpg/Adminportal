...
export async function POST(req) {
  try {
    ...
    for (const dataObject of dataArray) {
      ...
      const normalize = str => str.replace(/\s+/g, '_').toUpperCase()

      // Normalize the input
      const normalizedInput = normalize(dataObject.RegistrationType)

      // Check if the normalized input matches any valid type
      const matchedType = validRegistrationTypes.find(type => normalize(type) === normalizedInput)
      ...
      // Use the matched type for storing the correct value
      dataObject.RegistrationType = matchedType
      ...
      const genderValidationQuery = `SELECT COUNT(*) AS genderCount FROM dropdownmaster WHERE Id = ? OR DropDownValue = ? AND DropDownType = 'Gender'` // Replace with your actual gender table query
    ...
    // }
      ...
      const params = [
        ...
        dataObject.YearEstablished ?? '',
        dataObject.BusinessTurnOver ?? '',
